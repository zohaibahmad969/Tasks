...
global $wpdb;
		...
		if ('delete' === $table->current_action()) {
			$message = '<div class="updated below-h2" id="message"><p>' . sprintf(__('Device deleted: %d', 'custom_table_example'), count(intval($_REQUEST['ids']))) . '</p></div>';
		}
		?>
	<div class="wrap">
		...
		<div class="icon32 icon32-posts-post" id="icon-edit"><br></div>
		  ...
		  <form method="post">
                <input type="hidden" name="page" value="rdsn_app_devices_information" />
...
class Devices_List extends WP_List_Table {
	...
	public function prepare_items() {
		...
		if ( ! empty( $_REQUEST['s'] ) ) {
			...
			$sql .= " WHERE `device_id` = {$device_id} OR `device_token` = {$device_id} OR `os_type` = {$device_id}";

		} else {


			if ( ! empty( $_REQUEST['device_token'] ) ) {
				$commission_status = esc_sql( $_REQUEST['device_token'] );

				$status_filter = " AND `device_token` = '{$device_token}'";

				$sql .= $status_filter;
			}
			if ( ! empty( $_REQUEST['os_type'] ) ) {
				$commission_status = esc_sql( $_REQUEST['os_type'] );

				$status_filter = " AND `os_type` = '{$os_type}'";
	...
	public function get_views() {
		$views = array(
			'all' => '<li class="all"><a href="' . admin_url( 'admin.php?page=rdsn_app_devices' ) . '">' . esc_html__( 'Show All', 'app_device' ) . '</a></li>',
		);
	...
	public function get_columns() {
		$columns = array(
			...
			'device_token'            => __( 'Device Token', 'devices' ),
			'os_type'              => __( 'OS Type', 'devices' ),
			'date_created'        => __( 'Date', 'devices' ),
	...
	public function get_sortable_columns() {
        ...
        'device_id' => array( 'device_id', false ),
			'device_token' => array( 'device_token', false ),
			'os_type' => array( 'os_type', false ),
		);
	...
	public function process_bulk_action() {
        ...
        $table_name = $wpdb->prefix . 'themepark_device_configuration';
		if ( empty( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'bulk-app_devices' ) ) {
			return;
		}
