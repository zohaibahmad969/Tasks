...
exit();
	...
	//if( $_REQUEST["dateValue"] ) { $date_unformatted = $_REQUEST['dateValue']; }
...
<div id="rdsn-ajax-modal" class="trans-0-1">
    ...
    <div class="title-date"><?php echo $date_formatted; ?></div>
    ...
    <?php
		...
		$park_ot_custom = $day_row['rpt_ot_custom'];
		$day_content = $day_row['rpt_ot_notes'];
		$day_events = $day_row['rpt_ot_events'];
		$ticket_link =get_field('main_ticket_url','option');	
		//
		$complex_closed = $day_row['rpt_ot_complex_closed'];
		$complex_closed_message = get_field('complex_closed_message','option');
		if ($complex_closed != 'yes'):
		//
		if($park_ot_custom) {
			echo '<div class="content-ot">Our outdoor rides are open on '.$date_short.', <br />from '.$park_ot_custom.'.</div>';
		}
		else if ($park_ot_val != 0) {
			echo '<div class="content-ot">Our outdoor rides are open on '.$date_short.', <br />from '.$park_ot_label.'.</div>';
		} else {
			echo '<div class="content-ot">Our outdoor rides are closed today but the following attractions and events are open.</div>';	
		}
		//if ($park_ot_val != 0) {
		if ($park_ot_val != 0 || $park_ot_custom) { if ($ticket_link) { echo '<a href="'.$ticket_link.'" target="_blank" class="btn-solid ticket-link trans-0-2">Buy tickets</a>'; } }
		//
