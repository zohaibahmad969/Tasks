...
//jQuery('.banner .slide').fadeTo(1000,1);
...
/*// For use with latest version of Cycle 2. "fadeInUp" class needs to be removed from the banner function.
jQuery('#banner').on('cycle-before',function(event, optionHash, outgoingSlideEl, incomingSlideEl, forwardFlag) {
	jQuery(incomingSlideEl).find('.title-bar').addClass('fadeInUp');
	jQuery(outgoingSlideEl).find('.title-bar').removeClass('fadeInUp');
});*/
...
/*jQuery(window).on("resize", function(){
	jQuery('#rdsn-crsl').cycle('destroy');
	rdsnCarousel();
});*/
...
/*jQuery('#nav-mobile > ul > li').each(function() {
	if(jQuery(this).closest('li').find('ul').children().length != 0) {
		var $link = jQuery(this).find('a')[0].outerHTML;
		jQuery(this).closest('li').find('ul').prepend('<li class="page-item">'+$link+'</li>');
	}
});*/
...
jQuery('#nav-icon-wrapper,#nav-icon,#nav-mobile').on( 'click touchstart', function(e){
    e.stopPropagation();
});

jQuery(function() {  
	var nav_btn_wrapper = jQuery('#nav-icon-wrapper,#nav-icon'); 
	var nav_btn = jQuery('#nav-icon'); 
	var nav_menu = jQuery('#nav-mobile');
	jQuery(nav_btn_wrapper).on('click', function(e) {  
		e.preventDefault();
